...
class TripController extends Controller {
    ...
    /**
     * @api {post} /api/user-trip  User Trip
     * @apiHeader {String} Accept application/json.
     * @apiName PostUserTrip
     ...
     * @apiSuccess {String} success true 
     * @apiSuccess {String} status_code (200 => success, 404 => Not found or failed). 
     * @apiSuccess {String}  message Notification List.
     * @apiSuccess {JSON} data response.
     * @apiSuccessExample {json} Success-Response:
     * HTTP/1.1 200 OK
     *  {
    ...
    public function notificationList(Request $request) {
        try {
            ...
            $upcomingtrip = Trip::where('user_id',$user->id)->where('is_read',0)->whereDate('trip_date','=',date('Y-m-d'))->first();
            $trip = Trip::where('user_id',$user->id)->where('is_read',1)->whereDate('trip_date','=',date('Y-m-d'))->first();
            $data['upcoming'] = $upcomingtrip;
            $data['running'] = $trip;
            return $this->sendSuccessResponse("Trip List", $data);
        } catch (\Exception $e) {
            return $this->sendErrorResponse($e->getMessage(), (object) []);
        }
    }
