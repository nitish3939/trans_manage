...
class BalanceController extends Controller {
    ...
    public function index() {
        ...
        return view('subadmin.balance.index', ['js' => $js, 'css' => $css]);
    }
    ...
    public function balanceList(Request $request) {
        try {
            ...
            foreach ($balances as $user) {
                ...
                $usersArray[$i]['view-deatil'] = '<a class="btn btn-info btn-xs" href="' . route('subadmin.balance.edit', ['id' => $user->id]) . '"><i class="fa fa-pencil"></i>Edit</a>';
                $i++;
            }
    ...
    public function addBalance(Request $request) {
        try {
            if ($request->isMethod("post")) {
                    ...
                    if ($balance->save()) {
                        return redirect()->route('subadmin.balance.index')->with('status', 'Balance has been added successfully');
                    } else {
                        return redirect()->route('subadmin.balance.add')->with('error', 'Something went be wrong.');
                    }
                }
            ...
            return view('subadmin.balance.add', [
                'css' => $css,
                'js' => $js,

            ]);
        } catch (\Exception $ex) {
            return redirect()->route('subadmin.balance.index')->with('error', $ex->getMessage());
        }
    }
        ...
        try {
            ...
            if ($request->isMethod("post")) {
                ...
                if ($balance->save()) {
                    ...
                    return redirect()->route('subadmin.balance.index')->with('status', 'Balance has been updated successfully.');
                } else {
                    return redirect()->route('subadmin.balance.index', $id)->with('error', 'Something went be wrong.');
                }
            }
            $css = [
                "vendors/iCheck/skins/flat/green.css",
            ...
            return view('subadmin.balance.edit', [
                'css' => $css,
                'js' => $js,
                'balance' => $balance,
                    ]
