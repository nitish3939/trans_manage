...
class VehicleController extends Controller {
    ...
    public function index() {
        ...
        return view('subadmin.vehicle.index', ['js' => $js, 'css' => $css]);
    }
    ...
    public function issueViewVehicle(Request $request, $id) {
        try {
            ...
            if ($request->isMethod("post")) {
                ...
                if ($vehicle->save()) {
                    return redirect()->route('subadmin.vehicle.index')->with('status', 'Vehicle Issue has been updated successfully.');
                } else {
                    return redirect()->route('subadmin.vehicle.index', $id)->with('error', 'Something went be wrong.');
                }
            }
            ...
            return view('subadmin.vehicle.issueView', ["vehicle" => $vehicle
            ,'css' => $css,
    ...
    public function addVehicle(Request $request) {
        try {
            if ($request->isMethod("post")) {
                    ...
                    if ($vehicle->save()) {
                        return redirect()->route('subadmin.vehicle.index')->with('status', 'Vehicle has been added successfully');
                    } else {
                        return redirect()->route('subadmin.vehicle.add')->with('error', 'Something went be wrong.');
                    }
                }
            ...
            return view('subadmin.vehicle.add', [
                'css' => $css,
                'js' => $js,

            ]);
        } catch (\Exception $ex) {
            return redirect()->route('subadmin.vehicle.index')->with('error', $ex->getMessage());
        }
    }
    ...
    public function issueVehicle(Request $request, $id) {
        try {
            ...
            foreach ($vehicles as $user) {
                ...
                $usersArray[$i]['view-deatil'] = '<a class="btn btn-danger btn-xs" href="' . route('subadmin.vehicle.issueView', ['id' => $user->id]) . '"><i class="fa fa-pencil"></i>View</a>';
                $i++;
            }
            $data['data'] = $usersArray;
            ...
            return view('subadmin.vehicle.issue', [
                'css' => $css,
                'js' => $js,
        ...
        try {
            ...
            if ($request->isMethod("post")) {
                ...
                if ($vehicle->save()) {
                    ...
                    return redirect()->route('subadmin.vehicle.index')->with('status', 'Vehicle has been updated successfully.');
                } else {
                    return redirect()->route('subadmin.vehicle.index', $id)->with('error', 'Something went be wrong.');
                }
            }
            ...
            return view('subadmin.vehicle.edit', [
                'css' => $css,
                'js' => $js,
                'vehicle' => $vehicle,
                    ]
