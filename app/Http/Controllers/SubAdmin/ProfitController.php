...
class ProfitController extends Controller {
    ...
    public function index() {
        ...
        return view('subadmin.profit.index', ['js' => $js, 'css' => $css]);
    }
    ...
    public function profitList(Request $request) {
        try {
            ...
            foreach ($profits as $user) {
                ...
                $usersArray[$i]['view-deatil'] = '<a class="btn btn-info btn-xs" href="' . route('subadmin.profit.edit', ['id' => $user->id]) . '"><i class="fa fa-pencil"></i>Edit</a>';
                $i++;
            }
    ...
    public function addProfit(Request $request) {
        try {
            if ($request->isMethod("post")) {
                    ...
                    if ($profit->save()) {
                        return redirect()->route('subadmin.profit.index')->with('status', 'Profit has been added successfully');
                    } else {
                        return redirect()->route('subadmin.profit.add')->with('error', 'Something went be wrong.');
                    }
                }
            ...
            return view('subadmin.profit.add', [
                'css' => $css,
                'js' => $js,

            ]);
        } catch (\Exception $ex) {
            return redirect()->route('subadmin.profit.index')->with('error', $ex->getMessage());
        }
    }
        ...
        try {
            ...
            if ($request->isMethod("post")) {
                ...
                if ($profit->save()) {
                    ...
                    return redirect()->route('subadmin.profit.index')->with('status', 'Profit has been updated successfully.');
                } else {
                    return redirect()->route('subadmin.profit.index', $id)->with('error', 'Something went be wrong.');
                }
            }
            $css = [
                "vendors/iCheck/skins/flat/green.css",
            ...
            return view('subadmin.profit.edit', [
                'css' => $css,
                'js' => $js,
                'profit' => $profit,
                    ]
