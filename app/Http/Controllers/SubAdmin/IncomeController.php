...
class IncomeController extends Controller {
    ...
    public function index() {
        ...
        return view('subadmin.income.index', ['js' => $js, 'css' => $css]);
    }
    ...
    public function incomeList(Request $request) {
        try {
            ...
            foreach ($incomes as $user) {
                ...
                $usersArray[$i]['view-deatil'] = '<a class="btn btn-info btn-xs" href="' . route('subadmin.income.edit', ['id' => $user->id]) . '"><i class="fa fa-pencil"></i>Edit</a>';
                $i++;
            }
    ...
    public function addIncome(Request $request) {
        try {
            if ($request->isMethod("post")) {
                    ...
                    if ($income->save()) {
                        return redirect()->route('subadmin.income.index')->with('status', 'Income has been added successfully');
                    } else {
                        return redirect()->route('subadmin.income.add')->with('error', 'Something went be wrong.');
                    }
                }
            ...
            return view('subadmin.income.add', [
                'css' => $css,
                'js' => $js,

            ]);
        } catch (\Exception $ex) {
            return redirect()->route('subadmin.income.index')->with('error', $ex->getMessage());
        }
    }
        ...
        try {
            ...
            if ($request->isMethod("post")) {
                ...
                if ($income->save()) {
                    ...
                    return redirect()->route('subadmin.income.index')->with('status', 'Income has been updated successfully.');
                } else {
                    return redirect()->route('subadmin.income.index', $id)->with('error', 'Something went be wrong.');
                }
            }
            $css = [
                "vendors/iCheck/skins/flat/green.css",
            ...
            return view('subadmin.income.edit', [
                'css' => $css,
                'js' => $js,
                'income' => $income,
                    ]
