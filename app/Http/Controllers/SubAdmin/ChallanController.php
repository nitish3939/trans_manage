...
class ChallanController extends Controller {
    ...
    public function index() {
        ...
        return view('subadmin.challan.index', ['js' => $js, 'css' => $css]);
    }
    ...
    public function challanList(Request $request) {
        try {
            ...
            foreach ($vehicles as $user) {
                ...
                $usersArray[$i]['view-deatil'] = '<a class="btn btn-info btn-xs" href="' . route('subadmin.challan.edit', ['id' => $user->id]) . '"><i class="fa fa-pencil"></i>Edit</a>';
                $i++;
            }
    ...
    public function editChallan(Request $request, $id) {
        try {
            ...
            if ($request->isMethod("post")) {
                ...
                if ($challan->save()) {
                    return redirect()->route('subadmin.challan.index')->with('status', 'Challan has been updated successfully.');
                } else {
                    return redirect()->route('subadmin.challan.index', $id)->with('error', 'Something went be wrong.');
                }
            }
            $css = [
            ...
            $vehicle = Vehicle::all();
            return view('subadmin.challan.edit', [
                'css' => $css,
                'js' => $js,
                'challan' => $challan,
                'vehicle' => $vehicle,
            ...
            return redirect()->route('subadmin.challan.index')->with('error', $ex->getMessage());
        }
    }
