...
class TradingController extends Controller {
    ...
    public function index() {
        ...
        return view('admin.trading.index', ['js' => $js, 'css' => $css]);
    }
    ...
    public function tradingList(Request $request) {
        try {
            ...
            foreach ($tradings as $user) {
                ...
                $usersArray[$i]['view-deatil'] = '<a class="btn btn-info btn-xs" href="' . route('admin.trading.edit', ['id' => $user->id]) . '"><i class="fa fa-pencil"></i>Edit</a>';
                $i++;
            }
    ...
    public function addTrading(Request $request) {
        try {
            if ($request->isMethod("post")) {
                    ...
                    if ($trading->save()) {
                        return redirect()->route('admin.trading.index')->with('status', 'Trading has been added successfully');
                    } else {
                        return redirect()->route('admin.trading.add')->with('error', 'Something went be wrong.');
                    }
                }
            ...
            return view('admin.trading.add', [
                'css' => $css,
                'js' => $js,

            ]);
        } catch (\Exception $ex) {
            return redirect()->route('admin.trading.index')->with('error', $ex->getMessage());
        }
    }
        ...
        try {
            ...
            if ($request->isMethod("post")) {
                ...
                if ($trading->save()) {
                    ...
                    return redirect()->route('admin.trading.index')->with('status', 'Trading has been updated successfully.');
                } else {
                    return redirect()->route('admin.trading.index', $id)->with('error', 'Something went be wrong.');
                }
            }
            $css = [
                "vendors/iCheck/skins/flat/green.css",
            ...
            return view('admin.trading.edit', [
                'css' => $css,
                'js' => $js,
                'trading' => $trading,
                    ]
