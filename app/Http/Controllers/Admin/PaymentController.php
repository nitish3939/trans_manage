...
class PaymentController extends Controller {
    ...
    public function index() {
        ...
        return view('admin.payment.index', ['js' => $js, 'css' => $css]);
    }
    ...
    public function paymentList(Request $request) {
        try {
            ...
            foreach ($payments as $user) {
                ...
                $usersArray[$i]['view-deatil'] = '<a class="btn btn-info btn-xs" href="' . route('admin.payment.edit', ['id' => $user->id]) . '"><i class="fa fa-pencil"></i>Edit</a>';
                $i++;
            }
    ...
    public function addPayment(Request $request) {
        try {
            if ($request->isMethod("post")) {
                    ...
                    if ($payment->save()) {
                        return redirect()->route('admin.payment.index')->with('status', 'Payment has been added successfully');
                    } else {
                        return redirect()->route('admin.payment.add')->with('error', 'Something went be wrong.');
                    }
                }
            ...
            return redirect()->route('admin.payment.index')->with('error', $ex->getMessage());
        }
    }
        ...
        try {
            ...
            if ($request->isMethod("post")) {
                ...
                if ($payment->save()) {
                    ...
                    return redirect()->route('admin.payment.index')->with('status', 'Payment has been updated successfully.');
                } else {
                    return redirect()->route('admin.payment.index', $id)->with('error', 'Something went be wrong.');
                }
            }
            $css = [
                "vendors/iCheck/skins/flat/green.css",
